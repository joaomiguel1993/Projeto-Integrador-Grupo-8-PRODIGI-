...
def obter_por_username(username: str):
    return utilizadores_service.obter_por_username(username)
...
def criar(data: UtilizadorCreate):
    return utilizadores_service.criar_utilizador(data.model_dump())


@router.put("/{id_func}", response_model=UtilizadorOut)
def atualizar(id_func: int, data: UtilizadorUpdate):
    return utilizadores_service.atualizar_utilizador(id_func, data.model_dump(exclude_unset=True))
