...
def criar(data: HospitalCreate):
    return hospitais_service.criar_hospital(data.model_dump())


@router.put("/{id_hosp}", response_model=HospitalOut)
def atualizar(id_hosp: int, data: HospitalUpdate):
    return hospitais_service.atualizar_hospital(id_hosp, data.model_dump(exclude_unset=True))


@router.delete("/{id_hosp}")
def remover(id_hosp: int):
