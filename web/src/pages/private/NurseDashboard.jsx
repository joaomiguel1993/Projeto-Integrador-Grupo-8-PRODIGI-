...
export default function NurseDashboard() {
  ...
  const episodiosFiltrados = useMemo(() => ({
    sem: (episodios || []).filter((ep) =>
      mapEstado.sem(ep) && normalizar([ep.nome_utente, ep.nif_utente, ep.datahoraentr].join(' ')).includes(normalizar(filtrosSala.sem))
    ),
    em: (episodios || []).filter((ep) =>
      mapEstado.em(ep) && normalizar([ep.nome_utente, ep.nif_utente, ep.datahoraini_triagem || ep.datahoraentr].join(' ')).includes(normalizar(filtrosSala.em))
    ),
    triados: (episodios || []).filter((ep) =>
      mapEstado.triados(ep) && normalizar([ep.nome_utente, ep.nif_utente, ep.datahora_triagem || ep.datahoraentr].join(' ')).includes(normalizar(filtrosSala.triados))
    ),
    desistencias: (episodios || []).filter((ep) =>
      mapEstado.desistencias(ep) && normalizar([ep.nome_utente, ep.nif_utente, ep.datahora_desistencia || ep.datahoraentr].join(' ')).includes(normalizar(filtrosSala.desistencias))
    ),
    ...
    return (
      <section className="admin-panel-section">
        ...
        <div className="tabs" role="tablist" aria-label="Tabs Sala de Espera">
          {tabs.map((t) => (
            <button key={t.id} type="button" role="tab" aria-selected={salaTab === t.id} className={`admin-secondary-button ${salaTab === t.id ? 'is-active' : ''}`} onClick={() => setSalaTab(t.id)}>
